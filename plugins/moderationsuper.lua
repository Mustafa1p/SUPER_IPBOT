    ...
    for k,v in pairs(result.members) do
      if extra.matches[2] == tostring(v.id) then
        if extra.matches[1] == 'promote' then
          return promote('channel#id'..result.id, 'user#id'..extra.matches[2], tostring(v.id))
        elseif extra.matches[1] == 'demote' then
          return demote('channel#id'..result.id, 'user#id'..extra.matches[2], tostring(v.id))
        elseif extra.matches[1] == 'adminprom' then
          return admin_promote('channel#id'..result.id, 'user#id'..extra.matches[2], tostring(v.id))
        elseif extra.matches[1] == 'admindem' then
          return admin_demote('channel#id'..result.id, 'user#id'..extra.matches[2], tostring(v.id))
        end
      end
    end
    send_large_msg('channel#id'..result.id, 'No user user#id'..extra.matches[2]..' in this group.')
  end
end
...
local function action_by_reply(extra, success, result)
  ...
  if msg.to.type == 'channel' and not is_sudo(msg) then
...
function run(msg, matches)
    ...
    if is_admin(msg) then
      if matches[1] == 'promote' then
        ...
        if string.match(matches[2], '^%d+$') then
          channel_info(receiver, action_by_id, {msg=msg, matches=matches})
        elseif string.match(matches[2], '^@.+$') then
          local username = string.gsub(matches[2], '@', '')
          channel_info(receiver, username_id, {mod_cmd=matches[1], receiver=receiver, username=username})
        end
  ...
  if matches[1] == 'channel_add_user' and msg.action.user.id == our_id then
    return automodadd(msg)
  elseif matches[1] == 'channel_created' and msg.from.id == 0 then
    return automodadd(msg)
  end
end
